...
class WinterEnergyService:
    ...
    def __init__(self, db_path, config, snapshot_provider, log_callback=None):
        ...
        self._last_netatmo_diagnostic = None
        self._init_db()
    ...
    def _temperature_snapshot(self, netatmo):
        ...
        self._log_netatmo_diagnostic(stations, modules)
        ...
        for device in devices:
            ...
            device_fresh = self._cache_is_fresh(
                device.get("last_update") or device.get("updated_at"),
                max(900, int(netatmo.get("poll_seconds") or 300) * 3),
            )
            value = self._number(metric.get("value")) if cache_fresh and device_fresh and metric.get("online") else None
            if value is None:
                continue
            roles = list(patterns)
    ...
    def _hvac_snapshot(self, intesis_cache):
        ...
        for device in devices:
            ...
            error = bool(device.get("error")) or self._number(device.get("error_code")) not in (None, 0)
            if not driver_fresh or error:
                state = "offline"
        ...
        return result, ("online" if driver_fresh else "offline")
    ...
    def _log_netatmo_diagnostic(self, stations, modules):
        entries = []
        for kind, devices in (("station", stations), ("module", modules)):
            for device in devices.values():
                ...
                entries.append({
                    ...
                    "metric_keys": sorted((device.get("metrics") or {}).keys()),
                })
        signature = repr(entries)
        if signature == self._last_netatmo_diagnostic:
            return
        self._last_netatmo_diagnostic = signature
        self._log(f"Winter Netatmo cache diagnostic (no credentials/tokens): {entries}")
