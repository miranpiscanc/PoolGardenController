...
def garden_lights_timer_tick(cfg, current=None):
    current = current or now()
    sched = _runtime["scheduler"]
    timer_value = sched.get("garden_lights_until")
    if not timer_value:
        return
    try:
        timer_until = datetime.fromisoformat(timer_value)
    except (TypeError, ValueError):
        sched["garden_lights_until"] = None
        save_state()
        return
    if current < timer_until:
        return
    device_id, relay_number = garden_lights_relay(cfg)
    current_status = cached_relay_status(device_id, relay_number).get("active")
    if current_status is False:
        sched["garden_lights_until"] = None
        save_state()
        return
    status = set_one(cfg, device_id, relay_number, False, source="garden_timer")
    if relay_set_confirmed(status, False):
        sched["garden_lights_until"] = None
        record_event(
            relay_label(cfg, device_id, relay_number),
            f"Relay {relay_number}",
            "Garden lights timer expired",
            "Timer",
            "Automatic switch-off after configured duration"
        )
        log("GARDEN_LIGHTS timer terminato, luci spente")
        save_state()
...
def scheduler_tick():
    ...
    # Il timer luci è indipendente dalla modalità e dall'abilitazione della pompa.
    garden_lights_timer_tick(cfg, current)
...
def api_relay():
    ...
    if dev == garden_dev and relay == garden_relay and not status.get("ignored"):
        with _lock:
            _runtime["scheduler"]["garden_lights_until"] = None
            save_state()
    return jsonify({"ok": status.get("ok"), "status": status})


@app.route("/api/garden/lights/timer", methods=["POST"])
def api_garden_lights_timer():
    ...
    cfg = load_config()
    device_id, relay_number = garden_lights_relay(cfg)
    status = set_one(cfg, device_id, relay_number, True, source="garden_timer")
    if not relay_set_confirmed(status, True):
        return jsonify({"ok": False, "error": status.get("error") or "Relay activation failed", "status": status}), 503
    timer_until = now() + timedelta(minutes=minutes)
    with _lock:
        _runtime["scheduler"]["garden_lights_until"] = timer_until.isoformat(timespec="seconds")
        save_state()
    record_event(
        relay_label(cfg, device_id, relay_number),
        f"Relay {relay_number}",
        "Garden lights timer started",
        "Timer",
        f"Automatic switch-off in {minutes} minutes"
    )
    log(f"GARDEN_LIGHTS timer avviato per {minutes} minuti")
    return jsonify({
        "ok": True,
        "status": status,
        "minutes": minutes,
        "timer_until": timer_until.isoformat(timespec="seconds")
    })
