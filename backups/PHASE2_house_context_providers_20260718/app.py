...
def load_houses_metadata():
    ...
    try:
        with open(HOUSES_CONFIG_PATH, "r", encoding="utf-8") as file:
            configured = json.load(file).get("houses", [])
    except (OSError, ValueError, AttributeError):
        configured = DEFAULT_HOUSES
    ...
    for raw in configured:
        ...
        houses.append({
            ...
            "modules": [str(module) for module in modules if str(module).strip()],
        })
    return houses or [dict(house) for house in DEFAULT_HOUSES]


def house_metadata(house_id):
    return next((house for house in load_houses_metadata() if house["id"] == house_id), None)
...
def cached_goodwe_pv_production():
    cached = _runtime.get("goodwe")
    if not isinstance(cached, dict) or cached.get("status") != "online":
        return None
...
def solar_heating_tick():
    ...
    set_solar_next_temperature_check(state, None)
    pv_production = cached_goodwe_pv_production()
    water_temperature = water_temperature_snapshot()
    if water_temperature is None:
        reset_early_completion_timer(state)
...
def winter_snapshot_provider():
    ...
    return {
        ...
        "goodwe": goodwe_status_snapshot(),
        "intesis": {"devices": hvac_devices, "status": intesis_status},
    }
...
def cesclans_dashboard():
    return render_cesclans_dashboard()


@app.route("/opicina")
def opicina_dashboard():
    house = house_metadata("opicina") or DEFAULT_HOUSES[1]
    return render_template("opicina.html", house=house)
...
def history():
    cfg = load_config()
    lang = selected_language(cfg, persist_browser=True)
    return render_template("history.html", language=lang, back_href="/cesclans")
