(function () {
  ...
  function renderStatus(data) {
    ...
    set("winterNetatmo", sources.netatmo ? "Online" : "Unavailable");
    set("winterGoodwe", sources.goodwe ? "Online" : "Unavailable");
    set("winterIntesis", sources.intesis ? "Online" : "Unavailable");
  ...
  function renderHvac(devices) {
    ...
    devices.forEach((device) => {
      ...
      const state = document.createElement("strong"); state.textContent = device.online === false ? "Offline" : `${device.power ? "On" : "Off"} · ${device.mode || "--"}`;
  ...
  function drawChart(canvas, samples, series) {
    ...
    const all = series.flatMap((item) => samples.map((row) => Number(row[item.key])).filter(Number.isFinite));
    ...
    series.forEach((item) => { ctx.strokeStyle = item.color; ctx.lineWidth = 2; ctx.beginPath(); let open = false;
      samples.forEach((row, index) => { const value = Number(row[item.key]); if (!Number.isFinite(value)) { open = false; return; }
